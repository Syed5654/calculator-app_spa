...
const submitData = async () => {
  try {
    loading.value = true;
    await Api.post("/login", data.value).then((res) => {
        ...
        <div class="card-body">
          ...
          <div class="form-wrapper">
            ...
            <input type="email" name="email" id="email" class="form-input" placeholder="your@email.com"
              v-model="data.email" />
            <span class="validation-msg" v-for="message of validation_message.email" :key="`email-${message}`">{{ message
            }}</span>
          </div>
          <div class="form-wrapper">
            ...
            <span class="validation-msg" v-for="message of validation_message.password" :key="`password-${message}`">{{
              message }}</span>
          </div>
          <button class="btn btn-login" @click="submitData" :disabled="loading">
            {{ loading ? "Please Wait..." : "Login" }}
          </button>
        </div>
      </div>
      <router-link to="/signup" class="text-center create-account-link">Create a new account</router-link>
    </div>
  </div>
</template>
